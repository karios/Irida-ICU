...
namespace ICU.Net
{
    ...
    public sealed class UdpSend
        ...
        public void SendMessage(string ip, int port, string message)
        {
            ...
            Notifier.dataToUdp(ip, port, message);

                    
        }

        /// <summary>
        /// Broadcast a message to all the connected IVUs
        /// </summary>
        /// <param name="message"></param>
        public void broadcastMessage(string message)
        {
            foreach (IvuInfo ivu in ivuDB.values()){
                SendMessage(ivu.getIp(), ivu.getPort(), message);
            }
